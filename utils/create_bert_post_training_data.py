...
# from https://github.com/taesunwhang/BERT-ResSel/blob/2e9dd9e4b63f2eb4dcfa58036e100327db896e80/data/create_bert_post_training_data.py
...
class CreateBertPretrainingData(object):
  def __init__(self, args):
    self.args = args
    self.special_tok = args.special_tok.split(",")
    self._bert_tokenizer_init(self.special_tok)

  def _bert_tokenizer_init(self, special_tok, bert_pretrained='uncased_L-12_H-768_A-12'):
    bert_pretrained_dir = os.path.join("/remote_workspace/fusion_esim/data/pre_trained_ckpt", bert_pretrained)
  ...
  def read_raw_file(self, input_file):
    assert os.path.exists(input_file), "can't find input file"
    ...
    for doc in tqdm(raw_data):
      if doc[2] == 0:
        continue
      cat_doc = (doc[0] + doc[1]).split("__eot__")
      cat_doc = [self._add_special_tokens(self._bert_tokenizer.tokenize(x.strip().replace("__eou__", "[EOU]"))) for x in cat_doc]
      docs.append(cat_doc)
    return docs
  ...
  def create_training_instances(self, input_file, max_seq_length,
    ...
    all_documents = self.read_raw_file(input_file)
...
if __name__ == "__main__":
  ...
  arg_parser.add_argument("--special_tok", dest="special_tok", type=str, default="[EOT],[EOU]",
                          help="Special Token.")
  args = arg_parser.parse_args()
  # TODO 1. input raw data => split by __eot__, transfer to post_train_dataloader 2. add special token
  ...
  create_data.create_training_instances(
    ...
    args.short_seq_prob, args.masked_lm_prob, args.max_predictions_per_seq, rng, "[EOT]")