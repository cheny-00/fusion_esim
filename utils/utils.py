...
def masked_max(
    ...
    """
    if mask is None: return torch.max(vector, dim=dim, keepdim=keepdim)[0]
    replaced_vector = vector.masked_fill(~mask.unsqueeze(-1), min_value_of_dtype(vector.dtype))
...
def masked_mean(
    ...
    """
    if mask is None: return torch.mean(vector, dim=dim, keepdim=keepdim)
    mask = mask.unsqueeze(-1)
